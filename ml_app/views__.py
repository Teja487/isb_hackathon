...
# Path to your model and weights
MODEL_PATH = os.path.join(settings.PROJECT_DIR, 'models', 'model.json')
WEIGHTS_PATH = os.path.join(settings.PROJECT_DIR, 'models', 'model_weights.h5')
...
def entry_page(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'video':
            ...
            return redirect('ml_app:upload_image')  # Adjust the URL name accordingly
...
def upload_image(request):
    if request.method == 'GET':
        ...
        return render(request, 'upload_image.html', {'form': image_upload_form})

    if request.method == 'POST':
        image_upload_form = ImageUploadForm(request.POST, request.FILES)
        if image_upload_form.is_valid():
            # Get the uploaded image file and category
            uploaded_image = image_upload_form.cleaned_data['upload_image_file']
            category = image_upload_form.cleaned_data['image_category']
            
            # Save the image to the server
            image_name = f"uploaded_image_{int(time.time())}.jpg"
            ...
            # Normalize the image (use the same normalization as your training)
            img_array = img_array / 255.0  # Assuming you used [0, 1] normalization during training
            ...
            confidence = prediction[0][0] if prediction[0][0] < 0.33 else 1 - prediction[0][0]
